...
function EmptyState({ onPick }: { onPick: (text: string) => void }) {
  return (
    <div className="flex h-full flex-col items-center justify-center px-6 py-12 text-center">
      <span className="font-display text-2xl font-semibold tracking-tight text-ink">Ask FinMate anything.</span>
      ...
      <div className="mt-6 flex w-full max-w-md flex-col gap-2">
        {SUGGESTIONS.map((s) => (
          <button
            ...
            type="button"
            onClick={() => onPick(s)}
            className="flex items-center gap-2.5 rounded-xl border border-border bg-surface px-4 py-2.5 text-left text-sm text-ink-soft transition-colors hover:border-gold/40 hover:bg-gold-soft hover:text-gold-deep"
          >
...
export default function MessageList({
  ...
  return (
    <div className="themed-scroll flex-1 overflow-y-auto px-4 py-6 sm:px-8">
      {/* Centered, width-capped column -- see this redesign's layout
          notes: closer to ChatGPT's actual interaction pattern than the
          pre-redesign full-bleed layout, which stretched message bubbles
